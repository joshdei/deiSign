...
<body>
  ...
  <main class="main">
    ...
    <div class="file-info-bar" id="fileBar">
      ...
      <button class="btn-rm" onclick="reset()" title="Remove"><i class="fa-solid fa-xmark"></i></button>
    </div>

    <!-- Drop Zone -->
    <div class="dropzone card" id="dz">
      <div class="dz-icon-wrap"><i class="fa-solid fa-file-word"></i></div>
      <div class="dz-title">Drop your Word file here</div>
      <div class="dz-sub">or click to browse</div>
      <button class="btn-browse" onclick="document.getElementById('fileInput').click()">
        <i class="fa-solid fa-arrow-up-from-bracket"></i> Choose .docx File
      </button>
      <div class="dz-formats">Supports <span class="fmt">.docx</span> <span class="fmt">.doc</span></div>
      <input type="file" id="fileInput" accept=".docx,.doc" />
    </div>

    <!-- Options -->
    <div class="options-panel" id="optsPanel">
      <div class="opt">
    ...
    <div class="action-row" id="actionRow">
      <button class="btn-convert" id="cvtBtn" onclick="convert()">
        <i class="fa-solid fa-file-pdf"></i>
        <span id="cvtTxt">Convert to PDF</span>
        <div class="spin" id="cvtSpin"></div>
      </button>
      <button class="btn-opts" onclick="toggleOpts()">
    ...
    <div class="progress-wrap" id="progWrap">
      <div class="prog-head">
        <div class="prog-label">
          <i class="fa-solid fa-gear fa-spin" style="color:var(--gold);font-size:0.82rem"></i>
          <span id="progLbl">Processing…</span>
        </div>
        <span class="prog-pct" id="progPct">0%</span>
      </div>
      <div class="prog-track"><div class="prog-fill" id="progFill"></div></div>
      <div class="prog-steps">
        <div class="step" id="s1"><i class="fa-solid fa-file-arrow-up"></i>Reading file</div>
        <div class="step" id="s2"><i class="fa-solid fa-code"></i>Parsing content</div>
        <div class="step" id="s3"><i class="fa-solid fa-pen-ruler"></i>Rendering</div>
        <div class="step" id="s4"><i class="fa-solid fa-file-pdf"></i>Building PDF</div>
      </div>
    </div>
    ...
    <div class="success-wrap" id="successWrap">
      ...
      <div class="success-title">PDF Ready!</div>
      <div class="success-meta" id="successMeta">Your document has been converted successfully.</div>
      <a id="dlBtn" href="#" class="btn-dl" download>
        <i class="fa-solid fa-download"></i> Download PDF
      </a>
      <button class="btn-reset" onclick="reset()"><i class="fa-solid fa-arrow-rotate-left"></i> Convert another file</button>
    </div>

    <!-- Preview -->
    <div class="preview-section card" id="previewSection">
      <div class="preview-head">
        <div class="preview-ttl"><i class="fa-regular fa-file-lines"></i> Document Preview</div>
        <div class="preview-badge" id="pvBadge">—</div>
      </div>
      <div class="preview-body"><div id="docPreview"></div></div>
    </div>

    <!-- Features -->
    <div class="features">
      <div class="feat">
        <div class="feat-ico"><i class="fa-solid fa-shield-halved"></i></div>
        <div class="feat-title">100% Private</div>
        <div class="feat-desc">Files stay on your device. Nothing is ever sent to a server.</div>
      </div>
      <div class="feat">
        <div class="feat-ico"><i class="fa-solid fa-bolt"></i></div>
        <div class="feat-title">Instant</div>
        <div class="feat-desc">No upload wait time. Conversion runs entirely in your browser.</div>
      </div>
      <div class="feat">
        <div class="feat-ico"><i class="fa-solid fa-align-left"></i></div>
        <div class="feat-title">Formatting Kept</div>
        <div class="feat-desc">Headings, tables, lists, images — all carried through cleanly.</div>
      </div>
      <div class="feat">
        <div class="feat-ico"><i class="fa-solid fa-infinity"></i></div>
        <div class="feat-title">Free Forever</div>
        <div class="feat-desc">No sign-up, no limits, no watermarks. Always free to use.</div>
      </div>
    </div>
  ...
  <footer>
    <p>© 2025 pdf. — <a href="#">Privacy Policy</a> · <a href="#">Terms of Service</a> · All processing is done client-side in your browser.</p>
  </footer>
  ...
  <script>
    ...
    const cvtTxt      = document.getElementById('cvtTxt');
    const cvtSpin     = document.getElementById('cvtSpin');
    ...
    function handleFile(file) {
      if (!/\.(docx?|DOC[X]?)$/.test(file.name)) {
        showErr('Please select a .docx or .doc file.');
        return;
      }
      clearErr();
      currentFile = file;
      ...
      document.getElementById('fMeta').textContent = fmtBytes(file.size) + ' · Word Document';
    ...
    /* ─── Preview: mammoth → HTML ─── */
    async function parseForPreview(file) {
      try {
        const buf = await file.arrayBuffer();
        const result = await mammoth.convertToHtml({ arrayBuffer: buf });
        const html = result.value;

        docPreview.innerHTML = html || '<p style="color:var(--ink-soft);font-style:italic">No readable text found.</p>';
        previewSec.classList.add('show');

        const wc = (docPreview.innerText.trim().match(/\S+/g) || []).length;
        document.getElementById('pvBadge').textContent = wc.toLocaleString() + ' words';
      } catch (e) {
        docPreview.innerHTML = '<p style="color:var(--ink-soft);font-style:italic">Preview unavailable for this file.</p>';
        previewSec.classList.add('show');
      }
    }
    ...
    function toggleOpts() {
      optsOpen = !optsOpen;
      optsPanel.classList.toggle('show', optsOpen);
    }
    ...
    async function convert() {
      ...
      cvtTxt.textContent = 'Converting…';
      cvtSpin.classList.add('show');
      progWrap.classList.add('show');

      try {
        /* Step 1 */
        prog(12, 'Reading file…', 1);
        await wait(250);
        const buf = await currentFile.arrayBuffer();

        /* Step 2 */
        prog(32, 'Parsing Word content…', 2);
        await wait(200);
        const result = await mammoth.convertToHtml({ arrayBuffer: buf });
        const html = result.value;

        /* Step 3 */
        prog(58, 'Rendering layout…', 3);
        await wait(280);

        const ps   = document.getElementById('pageSize').value;
        const ori  = document.getElementById('orientation').value;

        const render = buildRender(html, ps, ori);
        document.body.appendChild(render);
        await wait(100); /* allow layout paint */

        /* Step 4 */
        prog(78, 'Generating PDF…', 4);
        ...
        const fmt = ps === 'a4' ? 'a4' : ps === 'a3' ? 'a3' : ps === 'letter' ? 'letter' : 'legal';
        ...
        while (y < totalH) {
          ...
          pdf.addImage(slice.toDataURL('image/jpeg', 0.94), 'JPEG', margin, margin, cW, srcH * ratio);
          y += pageH;
        }
        ...
        const outName = currentFile.name.replace(/\.(docx?|DOC[X]?)$/, '') + '.pdf';
        ...
        document.getElementById('successMeta').textContent =
          `"${outName}" · ${pages} page${pages !== 1 ? 's' : ''} · ${fmtBytes(blob.size)}`;

        progWrap.classList.remove('show');
        cvtBtn.disabled = false;
        cvtTxt.textContent = 'Convert to PDF';
        cvtSpin.classList.remove('show');
        ...
        cvtTxt.textContent = 'Convert to PDF';
        cvtSpin.classList.remove('show');
        showErr('Conversion failed: ' + (err.message || 'Unknown error. Try a different file.'));
      }
    }

    /* ─── Build off-screen render element ─── */
    function buildRender(html, ps, ori) {
      const sizes = {
        a4:     [794, 1123], letter: [816, 1056],
        a3:     [1123, 1588], legal: [816, 1344],
      };
      let [w, h] = sizes[ps] || sizes.a4;
      ...
      el.style.cssText = `
        ...
        font-size:14px; line-height:1.78; color:#1e293b;
      `;
      const style = document.createElement('style');
      style.textContent = `
        h1{font-size:26px;font-weight:700;margin:0 0 14px;color:#0f1117;font-family:'DM Serif Display',Georgia,serif}
        h2{font-size:20px;font-weight:600;margin:18px 0 10px;color:#0f1117;font-family:'DM Serif Display',Georgia,serif}
        h3{font-size:17px;font-weight:600;margin:14px 0 8px;color:#0f1117}
        p{margin:0 0 12px}
        ...
        blockquote{border-left:3px solid #c8963e;padding-left:14px;color:#5a607a;margin:10px 0}
        a{color:#c8963e}
      `;
    ...
    /* ─── Helpers ─── */
    function showErr(msg) { errorMsg.textContent = msg; errorBar.classList.add('show'); }
    function clearErr()   { errorBar.classList.remove('show'); }
